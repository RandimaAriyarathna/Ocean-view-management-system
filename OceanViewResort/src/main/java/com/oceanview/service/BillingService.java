...
/**
 * BillingService handles bill calculations for reservations.
 * Minor documentation updates were added for version control demonstration.
 */
public class BillingService {
    
    private ReservationDao reservationDao;
    private static final double TAX_RATE = 0.15; // 15% VAT applied to all reservations
    
    public BillingService() {
        this.reservationDao = new ReservationDao();
        System.out.println("✅ BillingService initialized successfully");
    }
    
    /**
     * Calculate bill for a reservation using reservation number
     */
    public BillCalculation calculateBill(String reservationNumber) {
        System.out.println("🔍 Calculating bill for reservation: " + reservationNumber);
        ...
        // -------- Reservation validation for billing eligibility --------
        ...
        if (!canGenerateBill) {
            if ("UPCOMING".equals(status)) {
                ...
                String errorMessage = "Bill cannot be generated for UPCOMING reservations. " +
                    ...
                    " (" + daysUntilCheckin + " day(s) remaining)";
        ...
        if ("ACTIVE".equals(status)) {
            ...
            System.out.println("   Partial billing calculated for: " + nights + " nights");
        } else {
            nights = ChronoUnit.DAYS.between(checkIn, checkOut);
            System.out.println("   Full stay billing calculated for: " + nights + " nights");
        }
        ...
        System.out.println("✅ Bill successfully calculated:");
        System.out.println("   Nights Stayed: " + nights);
        System.out.println("   Room Rate: Rs. " + String.format("%,.2f", roomRate));
        System.out.println("   Room Total: Rs. " + String.format("%,.2f", roomTotal));
        System.out.println("   Tax (15%): Rs. " + String.format("%,.2f", tax));
        System.out.println("   Final Bill Amount: Rs. " + String.format("%,.2f", totalBill));
    ...
    /**
     * Returns room rate based on room category
     */
    ...
    /**
     * Inner class used to store bill calculation results
     */
